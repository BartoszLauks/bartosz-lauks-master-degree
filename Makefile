...
migrate:
	docker-compose exec -T bartosz-lauks-master-degree-php php bin/console doctrine:migrations:diff --no-interaction
...
composer_install:
	COMPOSER_ALLOW_SUPERUSER=1 docker-compose exec -T bartosz-lauks-master-degree-php composer self-update
	COMPOSER_ALLOW_SUPERUSER=1 docker-compose exec -T bartosz-lauks-master-degree-php composer install --no-interaction --classmap-authoritative --optimize-autoloader
...
execphp:
	docker-compose exec bartosz-lauks-master-degree-php bash
